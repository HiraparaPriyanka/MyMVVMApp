...
data class Resource<out T>(val status: Status, val data: T?, val message: String?, val throwable: Throwable? = null, @IdRes val resId: Int? = null) {
    companion object {
        ...
        inline fun <reified T> error(@IdRes resId: Int? = 0, message: String? = null, throwable: Throwable? = null): Resource<T> {
            return Resource(status = Status.ERROR, data = null, message = message,resId = resId)
        }
